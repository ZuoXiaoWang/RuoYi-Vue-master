  ...
  <div class="app-container">
      ...
      <el-col :span="20" :xs="24">
        ...
        >
          ...
          <el-form-item label="性别" prop="personnelSex">
            <el-select
              v-model="queryParams.personnelSex"
              placeholder="请选择性别"
              clearable
            >
              <el-option
          ...
          <el-form-item label="状态" prop="personnelStatus">
            <el-select
              v-model="queryParams.personnelStatus"
              placeholder="请选择状态"
              clearable
            >
              <el-option
        ...
        >
          ...
          <el-table-column label="状态" align="center" prop="personnelStatus">
            <template slot-scope="scope">
              <dict-tag
                :options="dict.type.sys_staff_status"
                :value="scope.row.personnelStatus"
              />
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              <span>{{
                parseTime(scope.row.personnelEntryTime, "{y}-{m}-{d}")
              }}</span>
            </template>
          </el-table-column>
          <el-table-column
            label="离职时间"
            align="center"
            prop="personnelResignationTime"
            width="180"
          >
            <template slot-scope="scope">
              <span>{{
                parseTime(scope.row.personnelResignationTime, "{y}-{m}-{d}")
              }}</span>
            </template>
          </el-table-column>
              ...
              >
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item
                    ...
                    icon="el-icon-key"
                    v-hasPermi="['system:personnel:resetPwd']"
                    >重置密码
                  </el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="区域" prop="regionId" v-if="title == '添加员工'">
        ...
        <el-form-item
          v-if="form.personnelId == undefined"
          label="用户账号"
          prop="personnelLoginName"
        >
          <el-input
            v-model="form.personnelLoginName"
            placeholder="请输入用户账号"
          />
        </el-form-item>
        <el-form-item
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除员工管理编号为"' + personnelIds + '"的数据项？')
        .then(function () {
